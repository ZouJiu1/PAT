...
{
    {
        ...
        int number = 0;
        float* maxiou = (float *)xcalloc(l.max_boxes, sizeof(float));
        for (t = 0; t < l.max_boxes; ++t) {
            ...
            for (ln = 0; ln < l.total; ++ln) {
                int n = int_index(l.mask, ln, l.n);
                if (n >= 0) {
                    box pred = { 0 };
                    pred.w = l.biases[2 * ln] / state.net.w;
                    pred.h = l.biases[2 * ln + 1] / state.net.h;
                    for (j = 0; j < l.h; ++j) {
                        for (i = 0; i < l.w; ++i) {
                            pred.x = i / (float)(l.w);
                            pred.y = j / (float)(l.h);
                            ...
                            if(iou > 0.3) {
                                if(number==0) {
                                    mloam choose = {truth, iou, i, j, 1, ln, n, t};
        ...
        for(int ni = 0; ni < number; ni++) {
            ...
            int t          = mp.t;
            float iou_scale = mp.best_iou / maxiou[t];
            // printf("%f", iou_scale);
            int class_id = state.truth[t * l.truth_size + b * l.truths + 4];
            ...
            ious all_ious = delta_yolo_box(truth, l.output, l.biases, best_n, box_index, i, j, l.w, l.h, state.net.w, state.net.h, l.delta, (2 - truth.w * truth.h), l.w * l.h, l.iou_normalizer * class_multiplier * iou_scale, l.iou_loss, 1, l.max_delta, state.net.rewritten_bbox, l.new_coords);
            ...
            if (l.objectness_smooth) {
                float delta_obj = class_multiplier * l.obj_normalizer * (1 - l.output[obj_index]) * iou_scale;
                if (l.delta[obj_index] == 0) l.delta[obj_index] = delta_obj;
            }
            else l.delta[obj_index] = class_multiplier * l.obj_normalizer * (1 - l.output[obj_index]) * iou_scale;

            int class_index = entry_index(l, b, mask_n * l.w * l.h + j * l.w + i, 4 + 1);
            delta_yolo_class(l.output, l.delta, class_index, class_id, l.classes, l.w * l.h, &avg_cat, l.focal_loss, l.label_smooth_eps, l.classes_multipliers, l.cls_normalizer * iou_scale);
...
void forward_yolo_layer(const layer l, network_state state)
{
    ...
    *(l.cost) = 0;
    ...
    for (b = 0; b < l.batch; b++)
    {
        ...
        yolo_args[b].mloam_ptr = (mloam *)xcalloc(l.max_boxes * 10 + 60, sizeof(mloam));     // multi_label_one_anchor_choose_maxiou
