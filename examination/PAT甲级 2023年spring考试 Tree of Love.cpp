...
最后下午的时候发现是输出outer contour的问题，就AC了。判断是否对称要层序遍历，存在节点就vec保存1，
不存在vec保存0，若是该层的vec在reverse前后都相等，就是对称的。
判断是否是tree of love，要depth_first保存叶子节点，然后进行判断。
输出的Outer contour，保存叶子节点要放在最前面，然后是最左侧的节点，最后是后序遍历的最右侧节点。
最左侧和最右侧分别使用先left后right、先right后left的顺序进行递归遍历，以及不要重复，使用unordered_map来记录是否访问过。
*/
update   AC  codes
#include<iostream>
#include<vector>
...
reference: https://blog.csdn.net/SHSEDG/article/details/129433544
